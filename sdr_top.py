...
tx_msg_I,tx_msg_Q = sdr4.gen_bc13_syms()
...
rept = 4
W = base_len*rept
res,fstart = sdr5.packetDetect(utx_wn,L)
print("Packet detected",fstart)
#plt.plot(res)
...
corr = sdr5.frameAlignment(utx_wn[fstart-(base_len*L):fstart+(base_len*L)],ref_msg_corr)
plt.plot(np.abs(corr))
framestart = np.argmax(np.abs(corr)) + fstart - (base_len*L)
...
F0 = -0.002 # foffset*symbol_time of symbol rate
